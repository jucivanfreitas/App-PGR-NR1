...
- Before designing or changing any UI, consult the current project standards in `docs/ai-context/UI_PATTERNS.md` and `docs/ai-context/PROJECT_CONTEXT.md` to confirm the modal, table, toast, badge and action patterns already in use.
- Treat those docs as the first reference for layout, feedback and behavior decisions; only diverge when a concrete product requirement demands it.
- This skill helps implement UI and form behavior only; it does not define product boundaries, auth policy or multi-tenant rules.
- If UI guidance conflicts with official docs, follow the docs.
